...
class adbfs(toga.App):
    ...
    def startup(self):
        ...
        self.local_file_table = toga.Table(headings=["Name", "Type", "Size", "Date"], on_activate=self.on_local_file_activate, style=Pack(flex=1))
    ...
    async def on_remote_file_select(self, widget, row=None): # Add default value for row
        print(f"on_remote_file_select called with widget: {widget}, row: {row}")
        if row and self.last_selected_remote_row and hasattr(row, 'full_path') and hasattr(self.last_selected_remote_row, 'full_path') and row.full_path == self.last_selected_remote_row.full_path:
            await self.rename_selected_remote_file(widget)
            self.last_selected_remote_row = None # Reset to avoid accidental renames
        else:
            self.last_selected_remote_row = row
    ...
    async def refresh_local_file_list(self, widget=None):
        async with busy_cursor(self.main_window):
            ...
            try:
                ...
                self.sort_local_table(self.local_sort_column)
    ...
    async def refresh_remote_file_list(self, widget=None):
        ...
        async with busy_cursor(self.main_window):
            try:
                files = await self.loop.run_in_executor(None, self.adb_manager.get_file_list, self.current_remote_path)
                ...
                # Apply the current sort order after loading new data
                self.sort_remote_table(self.remote_sort_column)
    ...
    async def resolve_remote_path(self, path):
        for _ in range(10): # Limit to 10 levels of links to avoid infinite loops
            is_link = await self.loop.run_in_executor(None, self.adb_manager.is_link, path)
            if is_link:
                target = await self.loop.run_in_executor(None, self.adb_manager.get_link_target, path)
                if not target:
    ...
    async def on_remote_file_activate(self, widget, row):
        print(f"DEBUG: Remote file activated: {row.name if row else 'None'}") # Add debug print
        if not row:
            return
        ...
        is_dir = await self.loop.run_in_executor(None, self.adb_manager.is_directory, path_to_process)
    ...
    async def delete_selected_local_file(self, widget):
        if not self.local_file_table.selection:
            await self.main_window.info_dialog("Delete", "No file selected.")
            return

        row = self.local_file_table.selection
        confirmed = await self.main_window.confirm_dialog("Delete", f"Are you sure you want to delete {row.name}?")
        if confirmed:
            try:
                ...
                self.log_message(f"Error deleting local file: {e}")
                await self.main_window.error_dialog("Error", f"Could not delete file: {e}")
    ...
    async def upload_selected_local_file(self, widget):
        ...
        row = self.local_file_table.selection
        if row.type == "Folder":
            await self.main_window.info_dialog("Upload", "Cannot upload folders.")
            return
        ...
        async with busy_cursor(self.main_window):
            ...
            def upload_job():
                return self.adb_manager.push_file(local_path, remote_path, progress_handler)
            ...
            if success:
                ...
                await self.main_window.error_dialog("Error", "Upload failed.")
    ...
    async def create_remote_directory(self, widget):
        ...
        if folder_name:
            ...
            async with busy_cursor(self.main_window):
                ...
                success = await self.loop.run_in_executor(None, self.adb_manager.create_directory, remote_path)
                if success:
    ...
    async def delete_selected_remote_file(self, widget):
        if not self.current_device:
            await self.main_window.info_dialog("Error", "No device selected.")
            return
        if not self.remote_file_table.selection:
            await self.main_window.info_dialog("Delete", "No file selected.")
            return

        row = self.remote_file_table.selection
        confirmed = await self.main_window.confirm_dialog("Delete", f"Are you sure you want to delete {row.name}?")
        if confirmed:
            async with busy_cursor(self.main_window):
                self.log_message(f"Deleting remote file: {row.full_path}")
                success = await self.loop.run_in_executor(None, self.adb_manager.delete_file, row.full_path)
                if success:
                    self.log_message("File deleted.")
                    await self.refresh_remote_file_list()
                else:
                    self.log_message("Failed to delete file.")
                    await self.main_window.error_dialog("Error", "Failed to delete file.")
    ...
    async def rename_selected_remote_file(self, widget):
        ...
        if new_name and new_name != old_name:
            ...
            async with busy_cursor(self.main_window):
                self.log_message(f"Renaming remote file {old_path} to {new_path}")
                success = await self.loop.run_in_executor(None, self.adb_manager.rename_file, old_path, new_path)
    ...
    async def download_selected_remote_file(self, widget):
        ...
        row = self.remote_file_table.selection
        if row.type == "Folder":
            await self.main_window.info_dialog("Download", "Cannot download folders.")
            return
